    ...
    For i = 0 To (ActiveDocument.ModelSpace.Count - 1)
        ...
        If Entit.EntityName = "BlockInsert" Then
            ...
            If blk.Name = "JobData" Then
                ...
                Job.CalculatedByCompany = GetAttribOrDefault(blk, "", "CALCULATED_BY_COMPANY")
                Job.JobName = GetAttribOrDefault(blk, "", "JOB_NAME")
                Job.JobNumber = GetAttribOrDefault(blk, "", "JOB_NUMBER", "LEAD_NUMBER")
                Job.JobSiteLocation = GetAttribOrDefault(blk, "", "JOB_SITE_LOCATION", "SITE_LOCATION")
                ...
                Job.SupplyName = GetAttribOrDefault(blk, "", "SUPPLY_NAME")
                Job.SupplyStaticPressure = GetAttribOrDefault(blk, "0", "SUPPLY_STATIC_PRESSURE", "STATIC_PRESSURE")
                Job.SupplyResidualPressure = GetAttribOrDefault(blk, "0", "SUPPLY_RESIDUAL_PRESSURE", "RESIDUAL_PRESSURE")
                Job.SupplyAvailableFlow = GetAttribOrDefault(blk, "0", "SUPPLY_AVAILABLE_FLOW", "AVAILABLE_FLOW")
                Job.SupplyElevation = GetAttribOrDefault(blk, "0", "SUPPLY_ELEVATION", "METER_ELEVATION")
                Job.SupplyPipeLength = GetAttribOrDefault(blk, "0", "SUPPLY_PIPE_LENGTH", "METER_PIPE_LENGTH")
                Job.SupplyPipeSize = GetAttribOrDefault(blk, "0", "SUPPLY_PIPE_SIZE")
                Job.SupplyPipeType = GetAttribOrDefault(blk, "", "SUPPLY_PIPE_TYPE")
                Job.SupplyPipeInternalDiameter = GetAttribOrDefault(blk, "0", "SUPPLY_PIPE_INTERNAL_DIAMETER", "METER_PIPE_INTERNAL_DIAMETER")
                Job.SupplyPipeFittingsSummary = GetAttribOrDefault(blk, "", "SUPPLY_PIPE_FITTINGS_SUMMARY")
                ...
                Job.WaterFlowSwitchMakeModel = GetAttribOrDefault(blk, "", "WATER_FLOW_SWITCH_MAKE_MODEL")
